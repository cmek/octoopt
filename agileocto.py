...
class OctopusAgile:
    def __init__(self, product_code="AGILE-24-10-01", tariff_code="E-1R-AGILE-24-10-01-G", day=1):
...
def main():
    ...
    data = api.get_input_data()

    battery_charge_window = api.get_battery_charge_window()
    peak_time_window = api.get_peak_time()

    print(f"battery charge window: {battery_charge_window['start_time']} - {battery_charge_window['end_time']}")
    print(f"battery charge cost: {battery_charge_window['total_cost']}p (@ {battery_charge_window['avg_cost_per_khw']} p/kWh)")

    print(f"peak time window: {peak_time_window['start_time']} - {peak_time_window['end_time']}")
    print(f"peak time cost: {peak_time_window['total_cost']}p (@ {peak_time_window['avg_cost_per_khw']} p/kWh)")
    print(f"savings: {peak_time_window['total_cost'] - battery_charge_window['total_cost']}p")
